## Hi, I am Ketki Kale👋

💻Power BI Developer|📊Data Visualization Enthusiast|🚀Transforming data into actionable insights
_________________________________________________________________________________________________________________________________________________________________________________________________

# 🔗👩‍💻About Me
* 💼Growing into Data Analytics, positively focused on Power BI development.
* 🎓Completed internships in Power BI domain.
* 📊 Experienced in building dashboards & reports using **Power BI, DAX, and Power Query**
* 🔎 Interested in **data visualization, business intelligence, and analytics**
* 🌱 Learning more about **EDA (Exploratory Data Analysis)** and expanding into Python for data projects
* 📝 Completed projects like **Zomato Case Study: Food Delivery Analysis**
* 🔎Passionate about solving problems and finding solutions through data insights.
* 🤝Looking for remote roles in Power BI development, busines/data analytics

---

## 🛠️ Tech Stack

* **BI Tools:** Power BI, DAX, Power Query
* **Data Handling:** Excel, SQL (basics), Data Cleaning & Modeling
* **Analytics:** Exploratory Data Analysis (EDA), Data Visualization
* **Other Skills:** Dashboard Design, KPI Reporting

---

## 📌 Featured Projects

🔹 [Zomato Case Study: Data Analysis in Food Delivery Business](#)
🔹 [Retail Sales EDA Project (Jupyter Notebook)](#)
🔹 [Power BI Dashboards & Reports](#)

---

## 📫 Connect with Me

* 💼 [LinkedIn](www.linkedin.com/in/ketkikale)
* 📧 [ketki.kale.office@gmail.com]
* 🌐 Portfolio ()

---

✨ *“Data is the new oil, and I love refining it into insights that matter.”*
